...
本專案為 **APK 安全分析平台原型 (Prototype)**，包含：

-   **apk-platform**：FastAPI backend\
    負責上傳 APK、建立 `request.json`、呼叫分析模組、回傳 `report.json`

-   **AI-model**：AI / rule-based analysis engine\
    負責讀取 `request.json` 並輸出 `report.json`
